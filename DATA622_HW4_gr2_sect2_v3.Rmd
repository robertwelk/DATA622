...
- 12 years of education, completed high school, is the most common level of educational attainment. More participants had not completed high school than had a college education. Education levels ranged from 6 years to 19 years.
...
## 2. ADHD self-report
The raster plot below shows and aggregation of number of counts for each question/response pair for the ADHD set.  
...
df %>%
  ...
  labs(x="ADHD Total Score", y="Mood Disorder Total Score", title="Scatterplot of Mood Disorder Score vs ADHD Total Score with Suicide Attempts") + 
  theme_minimal()
...
df %>% 
  ...
  theme_minimal()+
  scale_fill_viridis_d(name="Question Response", labels=c("0-Never","1-Rarely","2-sometimes", "3-often","4-very ofter"),option="plasma") +
    labs(x="ADHD Question ID", 
         y="Pecentage of Respondants with Suicide Attempt", 
         title="Percentage of Respondants with Suicide Attempt by ADHD Question/Response Pair"
         )+
      theme(axis.text.x = element_text(angle = -90))+
...
df %>% 
  ...
  scale_fill_viridis_d(name="Question Response", labels=c("Yes","3-Serious"),option="plasma") +
    labs(x="Mood Disorder Question ID", 
         y="Pecentage of Respondants with Suicide Attempt", 
         title="Percentage of Respondants with Suicide Attempt by Mood Disorder Question/Response Pair"
         )+
      theme(axis.text.x = element_text(angle = -90))+
  facet_wrap(~type, scales="free")
```
...
- For the Mood Disorder questions, reponses of Yes for Q1 and Q2 and Serious for Q3 were the groups most associated with suicide   


Below, suicide is examined in relation to sex, race, age, and educational attainment, first in a scatterplot between Age and Education and next in a barplot of Race and Sex where group rate of suicide attempt is calculated.
...
df %>% filter(!is.na(Suicide)) %>%
  ...
  labs(x="", title = 'Legal Issue Coincidence with Suicide Attemp')+
  theme_minimal()+
  facet_wrap(~name, scales="free")  
...
First, let's use some predictive mean matching to impute missing data.
...
Now that there's no missing data, let's create some new variables to help simplify dataset, and reduce multi-class variables to binary ones so some of the algorithms work better.
...
In this section, two unsupervised learning techniques will be considered, clustering and principle componant analysis.
...
The results of hierarchical clustering were evaluated by aggregating the cluster label and variables and taking the mean value of each group. The means of each group were used to determine defining characteristics relavtive to the other cluster. A summary is provided:

Cluster 1, n=50: This cluster was most associated with suicide and were most likely to be white, females, victims of abuse, have two or more substance diagnosis, and two or more pysch meds. They were likely to self report ADHD but not necessarily mood disorder. They were also the youngest of the groups.
...
These PCs tend to have their correlations spread out amongst several variables; no one variable really takes precedence in any of these.  That being said, we can still describe these PCs in terms of the main variables they are comprised of.  PC1 for instance is primarily comprised of drug use and substance diagnosis variables, where the drug use variables are negatively correlated and the substance diagnosis ones are positively correlated.  PC2 is mostly comprised of psychological variables, and is highly correlated to psychotropic prescriptions, mood disorders, and somewhat to ADHD.  PC3 is positively correlated to being abused and suicide, while negatively correlated to psych meds.  PC4 is highly negatively correlated to violence and disorderly conduct and male sex.

The patterns are interesting, although there is definitely some colinearity between the variables within each PC (but not between PCs).  That being said, we could try to label the first four PCs as:
...
Below, 20% of the dataset is split off into a test set, while the remaining 80% becomes the training set (the models are trained on this).  This is done for both datasets.
...
```
 ...
 There are four hyperparameters that are to be tuned for this model, and a brief description of each is provided as well as the values that were supplied to the grid for cross validation purposes for the model tuning effort:
...
3.  *Learning rate (shrikage)*: This parameter sets the rate at which a tree learns from the base learner. Smaller rates learn less quickly thus reducing the chances of overfitting but will require a larger number of trees and computation power. Values 0.001, 0.01, and 0.1 were tested, representing slow, moderate, quick learners respectively. 
...
The final values for hyperparametes determined via cross-validation to optimize model output were n.trees = 1300, interaction.depth = 4, shrinkage =0.01 and n.minobsinnode = 1. The overall accuracy is 79.4% which is greater than the no-information rate, 70.6%. A balance was achieved between true positive and true negative rates, indicated by Sensitivity and Specificity values of 80.0% and 83.3%, respectively. 
...
Above is the breakdown of the model, and it's performance on a holdout set.  A cost value of about 15 was chosen.  This produces a mix of false/true positives/negatives, with an outsized amount of false negatives.  Unfortunately, it's prediction accuracy on the test set is right at the no-information rate.  Also, although this model have a high specificity (true negative rate), due to the nature of suicide prediction (and the implied impetus to interdict), it would be more desirable to have a high sensitivity (true positive rate).  
...
I suspect this data is not well separable, which may be impacting the performance of these SVM models.  Also, because the dataset is fairly unbalanced, we would need an overall accuracy of over 85% for the model to be significantly better than guessing 'no suicide' in each instance; this would likely prove difficult with this dataset even given optimal pre-processing an feature engineering.  
